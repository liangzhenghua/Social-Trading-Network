...
unique(cube.rb[, .(cube.type, n = .N / (as.numeric(max(date) - min(date)))), keyby = .(cube.symbol)
    ][is.finite(n)])[, .(min = min(n, na.rm = T), q25 = quantile(n, 0.25, na.rm = T) * 7, mean = mean(n, na.rm = T) * 7, median = median(n, na.rm = T) * 7, q75 = quantile(n, 0.75, na.rm = T) * 7, max = max(n, na.rm = T)), keyby = .(cube.type)]
